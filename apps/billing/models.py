...
class BillingRun(models.Model):
    ...
    billing_type = models.CharField(
        max_length=20, choices=BILLING_TYPE_CHOICES, default="manual"
    )
    ...
    rate_card_applied = models.ForeignKey(
        "rate_cards.RateCard", on_delete=models.SET_NULL, null=True, blank=True
    )
...
class BillingRunLineItem(models.Model):
    ...
    billing_run = models.ForeignKey(
        BillingRun, related_name="line_items", on_delete=models.CASCADE
    )
...
class BillingRunAttachment(models.Model):
    ...
    billing_run = models.ForeignKey(
        BillingRun, related_name="attachments", on_delete=models.CASCADE
    )
...
class BandTable(models.Model):
    ...
    # Stores the band calculation details as JSON
    band_data = models.JSONField(default=dict, blank=True)
    ...
    class Meta:
        indexes = [
            models.Index(fields=["ticket_number", "customer"]),
        ]
...
class FinalTicket(models.Model):
    ...
    # Stores the final processed row/table data as JSON
