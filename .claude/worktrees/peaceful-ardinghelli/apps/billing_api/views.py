...
# Initialize Supabase client (lazy — only if credentials are configured)
supabase = None
try:
    from supabase import create_client
    _url = getattr(settings, 'SUPABASE_URL', None)
    _key = getattr(settings, 'SUPABASE_KEY', None)
    if _url and _key:
        supabase = create_client(_url, _key)
except ImportError:
    pass  # supabase package not installed — API endpoints will return 503
...
def remove_workflow_cycle(request):
    ...
    if supabase is None:
        return JsonResponse({"success": False, "error": "Supabase not configured"}, status=503)
    ...
    results = []  # 👈 collect per-cycle results here
    ...
    for cycle_id in cycle_ids:
        ...
        try:
            ...
            # 3️⃣ Delete DB rows for this cycle
            delete_result = (
                supabase
                .table("workflow_executions")
                .delete()
